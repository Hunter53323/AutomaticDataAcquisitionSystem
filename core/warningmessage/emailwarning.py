...
class EmailWarning:
    def __init__(self) -> None:
        ...
        self.receiver: str = read_json("receiver")
    ...
    def send_email(self, subject: str, message: str) -> bool:
        ...
        smtp_server, port = self.get_smtp_server(self.receiver)

        # 连接到SMTP服务器并发送邮件
        try:
            server = smtplib.SMTP_SSL(smtp_server, port)
            server.login(self.sender_mail, self.sender_passwd)  # 登录验证
            server.sendmail(self.sender_mail, [self.receiver], msg.as_string())  # 发送邮件
            return True
    ...
    def check(self) -> bool:
        if self.sender_mail and self.sender_passwd and self.receiver:
            return True
        return False
