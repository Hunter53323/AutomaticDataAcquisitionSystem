...
def handle_command(data):
    ...
    if data[2].to_bytes() == b'\x01':
        byte2 = b'\x01'
        byte3 = b'\x01'
        data_checksum = calculate_checksum(data[0].to_bytes(), data[2].to_bytes(), b'\x01', b'\x01', data[3].to_bytes(), data[4:13])
        if data_checksum == data[13].to_bytes():
            byte4 = b'\x01'
        else:
            byte4 = b'\x02'
        byte5 = calculate_checksum(byte0, byte2, b'\x01', b'\x01', byte3, byte4)
        # 暂时不考虑校验和的正确与否
        ...
        byte14 = b'\x00'
        byte15 = b'\x00'
        byte16 = calculate_checksum(byte0, byte2, b'\x01', b'\x01', byte3, byte4and5+byte6and7+byte8and9+byte10and11+byte12and13+byte14+byte15)
        byte17 = b'\xA5'
...
if __name__ == "__main__":
    # 配置串行端口
    ser = serial.Serial('COM3', 9600, timeout=1)  # 请替换为您的串行端口和波特率
    print("服务器启动")
