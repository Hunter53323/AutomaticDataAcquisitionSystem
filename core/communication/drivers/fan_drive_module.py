...
class FanDriver(DriverBase):
    ...
    def read_all(self, read_count:int = 0) -> bool:
        ...
        # 检查校验和是否相符
        response_checksum = self.__calculate_checksum(response[0], response[2], b'\x01', b'\x01', response[3], response[4:13])
        if response[13] != response_checksum:
            if read_count == 3:
                return False
            return self.read_all(read_count=read_count+1)
        # 数据检查完毕后开始读取数据
        target_speed, actual_speed, dc_bus_voltage, U_phase_current, power, breakdown, status = self.__decode_response(response)
        if status == True:
            self.curr_data = {
    ...
    def write(self, para_dict: dict[str, any], write_count:int = 1) -> bool:
        ...
        command_code = b'\x01'
        total_package = b'\x01'
        current_package = b'\x01'
        data_length = b'\x03'
        bytes_data = byte5and6 + byte7and8 + byte9and10 + byte11and12
        checksum = self.__calculate_checksum(frame_header, command_code, total_package, current_package, data_length, bytes_data)
        frame_end = b'\x5A'

        byte0to4 = bytes.fromhex(data0 + data1 + data2 + data3)

        write_bytes = bytes.fromhex(data0 + data1 + data2 + data3) + bytes_data + checksum + frame_header
        ...
        # 根据校验和和校验结果判断是否成功
        response_checksum = self.__calculate_checksum(response[0], response[2], b'\x01', b'\x01', response[3], response[4])
        if response[5] == response_checksum:
            if response[4] == b'\x01':
                return True
            else:
                if write_count == 3:
            ...
            return False
    def __decode_response(self, response: bytes) -> tuple[int, int, int, int, int, list, bool]:
        ...
        response_data = response[4:16]
        if self.__calculate_checksum(frame_header, command_code, total_package, current_package, data_length, response_data) == response[16]:
            return target_speed, actual_speed, dc_bus_voltage, U_phase_current, power, breakdown, True
        else:
    ...
    def __calculate_checksum(*args: bytes) -> bytes:
        # 初始化校验和为0
        checksum = 0
